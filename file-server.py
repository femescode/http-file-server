...
class HTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # 列出文件处理
            ...
            html_sb.append('<li>上传命令：<code>find test.txt -maxdepth 1 -type f|while read l;do n=$(basename "$l"|xxd -ps|sed "s/../%%&/g");curl "http://%s:%s/%s$n" --data-binary @"$l"|cat; done</code></li>'%(localip,port,dirname))
            html_sb.append('</ol>')
            html_sb.append('<hr>')
            html_sb.append('<ul>')
